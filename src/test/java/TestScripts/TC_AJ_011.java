...
public class TC_AJ_011 extends BaseClass {

    @Test(dataProvider = "AjioData_TC_000", dataProviderClass = AjioLogin_DataProvider.class)
    public void verifyTotalSavings( String ITEM) throws InterruptedException {

        ItemsPage itemsPage = new ItemsPage(driver);
        CommonMethods commonMethods = new CommonMethods(driver);
        CartPage cartPage = new CartPage(driver);

        test = extent.createTest("TC_AJ_011 - Verify Total Savings vs (Discount + Coupon)");
       ...
       // commonMethods.login(EMAIL, PASSWORD); //login method for signing in
        ...
        Assert.assertEquals(cartPage.totalSaving(), cartPage.discount() + cartPage.couponSaving()); //assert method

        cartPage.doneSavingsButton();
    }
}